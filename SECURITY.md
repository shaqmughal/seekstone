...
- **Only modifies your vault through explicit tool calls** — the 10 write tools: `create_note`, `delete_note`, `move_note`, `rename_heading`, `append_note`, `patch_note`, `patch_frontmatter`, `replace_in_note`, `append_periodic_note`, `undo_write`. Every one of them journals the pre-image of each file it touches under `<vault>/.seekstone/history/` first, so `undo_write` can restore it byte-for-byte (`SEEKSTONE_HISTORY=0` disables the journal). One read tool has a documented write side effect: `get_periodic_note` creates the periodic note when `createIfMissing` is set (neutralized in read-only mode). It never makes background edits.
- **Can be locked down further.** `SEEKSTONE_READ_ONLY=1` runs the server read-only — the 10 write tools are removed from the advertised tool list entirely and rejected at dispatch if called anyway. `SEEKSTONE_WRITE_PATHS` (comma-separated vault-relative globs, e.g. `journal/**,inbox/*.md`) restricts writes to matching paths while the rest of the vault stays read-only. Both are enforced at the dispatch layer plus a shared per-handler check, so a new tool cannot forget them.
- **Leaves a receipt for every write.** With `SEEKSTONE_AUDIT_FILE` set, every write-tool call — including ones refused by policy or compare-and-swap — appends one JSON-line record (tool, paths, sha-256 before/after, outcome, op metadata; never note content) fsync'd after the vault write commits. An unwritable audit file fails boot and a failed append surfaces as a structured `audit_failed` error, so an unauditable write never passes silently.
