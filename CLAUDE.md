...
The **server** reads its own set (`SEEKSTONE_VAULT`, `SEEKSTONE_READ_ONLY`, `SEEKSTONE_WRITE_PATHS`, `SEEKSTONE_LOG_LEVEL`, `SEEKSTONE_LOG_FILE`, `SEEKSTONE_LOG_MAX_SIZE`, `SEEKSTONE_WATCH_POLL`, `SEEKSTONE_WATCH_POLL_INTERVAL`, `SEEKSTONE_SEMANTIC`, `SEEKSTONE_MODEL_PATH`, `SEEKSTONE_CACHE_DIR`) — documented in the README's Configuration table. Write behavior to know before editing tools: deletes are recoverable (`.trash/`), every write tool supports optional `prevHash` compare-and-swap (moves and deletes included since 0.14.0; every mutating result returns a `contentHash`), moves rewrite inbound links, and every write is policy-gated + atomic (see `docs/ARCHITECTURE.md` §2).
