...
| `SEEKSTONE_MODEL_PATH` | No | Directory holding the embedding model (default: where `fetch-model` puts the selected model, under the cache dir). Accepts a Model2Vec directory, or — **opt-in** — a HuggingFace-style ONNX transformer directory (`config.json` + `onnx/`, e.g. a local clone of `Xenova/bge-small-zh-v1.5`), for vaults where Model2Vec's static embeddings fall short (CJK especially). The ONNX path needs the optional `@huggingface/transformers` peer dependency installed alongside seekstone (`npx -y -p seekstone -p @huggingface/transformers seekstone`; ~350 MB unpacked with its ONNX runtime, native modules included — the default install stays dependency-free). Expect roughly 140 ms per query embedding and a few minutes for the first index of a thousand notes, against Model2Vec's single-digit milliseconds. No MaxSim rerank on this path. Either way the model is never downloaded at runtime — fetch it out-of-band first. |
