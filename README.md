...
| `SEEKSTONE_SEMANTIC_MODEL` | No | Which local model to load: `potion-base-8M` (default, ~30 MB, 256-dim), `potion-retrieval-32M` (~129 MB, 512-dim — more accurate on description-style queries at roughly 2× the query latency), or `M2V_multilingual_output` (~513 MB, 256-dim — LaBSE-distilled, 101 languages including CJK; the two potion models are English-centric, so non-English vaults should pick this one). Fetch non-default models first, e.g. `npx -y seekstone fetch-model --model M2V_multilingual_output`. |
