...
- **Local semantic search, fully in-process.** With `SEEKSTONE_SEMANTIC=1`, `search` gains `mode: "semantic"` and `"hybrid"` — meaning-based retrieval through a small on-device embedding model (one-time `npx -y seekstone fetch-model` download; the running server never touches the network). On our committed 10k-note benchmark vault, description-style queries hit the right note in the top 5 **70% of the time vs 30% for keyword search** ([committed eval](packages/harness/fixtures/baseline-reports/retrieval-eval.md)). No other server we **benchmarked** ships offline, zero-native-dependency embeddings — and we measured the alternatives head-to-head on the same golden set ([committed comparison](packages/harness/fixtures/baseline-reports/retrieval-eval-competitors.md)): obsidian-tc's Ollama-backed semantic search is genuinely **more accurate** (90% vs our 72% overall hit@5 — its 137M-parameter model vs our 8M one), and it pays for it with **12× slower queries** (165 ms vs 14 ms p50), **71× slower indexing** (26.5 min vs 22 s cold), **~7× the payload** (15.4 KB vs ~2 KB per query), and a second server you must install and run. obsidian-mcp-pro could not index the 10k-note vault at all (its JSON vector store exceeds JavaScript's string limit after ~15 minutes of embedding). Pick your trade — the numbers are all committed.
