...
| `SEEKSTONE_MODEL_PATH` | No | Directory holding the embedding model (default: where `fetch-model` puts the selected model, under the cache dir). Accepts a Model2Vec directory **or** a HuggingFace-style ONNX transformer directory (one containing `config.json` + `onnx/`, e.g. a local clone of `Xenova/bge-small-zh-v1.5` — loaded via `@huggingface/transformers` with `dtype: q8`; recommended for CJK vaults, where Model2Vec's static embeddings discriminate poorly). The model is never downloaded at runtime — fetch it out-of-band first. |
