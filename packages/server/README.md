...
| `SEEKSTONE_MODEL_PATH` | no | Directory holding the embedding model (default: where `fetch-model` puts the selected model). Model2Vec by default; opt-in, a HuggingFace-style ONNX directory (`config.json` + `onnx/`) loads through the optional `@huggingface/transformers` peer dependency — install it alongside seekstone; ~350 MB, native modules, ~140 ms per query. |
