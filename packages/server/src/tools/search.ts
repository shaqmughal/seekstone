...
/**
 * The single retrieval path behind `search` and `context_pack`. Async because
 * a transformer embedder (transformers.js) embeds the query through an ONNX
 * session; the lexical and Model2Vec paths never actually suspend, so their
 * results are byte-identical to the previous synchronous implementation.
 */
export async function retrieve(ctx: ServerContext, input: SearchInput): Promise<Retrieval> {
...
export async function search(ctx: ServerContext, input: SearchInput): Promise<SearchHit[]> {
  return (await retrieve(ctx, input)).hits;
}
...
async function semanticSearch(
  ...
  const queryVec = await semantic.embedQuery(input.query);
