...
describe('search', () => {
  it('basic search returns hits with path, title, score, excerpt, tags', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    const hits = await search(ctx, { query: 'roadmap', limit: 10 });
  ...
  it('omits title when it equals the path basename', async () => {
    const ctx = buildCtx('/vault', [
      { id: 'notes/widget.md', title: 'widget', body: 'a widget gadget thing' },
    ]);
    const hits = await search(ctx, { query: 'widget', limit: 10 });
    expect(hits[0]?.path).toBe('notes/widget.md');
    expect(hits[0]?.title).toBeUndefined();
  });

  it('keeps title when it differs from the basename', async () => {
    const ctx = buildCtx('/vault', [
  ...
  it('omits tags when the note has none', async () => {
    const ctx = buildCtx('/vault', [
  ...
  it('rounds score to at most 2 decimal places', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    const hits = await search(ctx, { query: 'roadmap', limit: 10 });
  ...
  it('excerptLength caps the excerpt and a larger value returns more context', async () => {
    ...
    const short = await search(ctx, { query: 'TARGET', limit: 10, excerptLength: 40 });
    const long = await search(ctx, { query: 'TARGET', limit: 10, excerptLength: 400 });
    expect(len(short[0]?.excerpt)).toBeLessThanOrEqual(40 + slack);
    expect(len(long[0]?.excerpt)).toBeGreaterThan(len(short[0]?.excerpt));
  });

  it('defaults excerptLength to 120 when omitted', async () => {
    const longBody = `intro ${'x '.repeat(400)}TARGET${' y'.repeat(400)} outro`;
    const ctx = buildCtx('/vault', [{ id: 'notes/long.md', title: 'long', body: longBody }]);
    const len = (s?: string) => (s ?? '').replace(/…/g, '').length;
    const hits = await search(ctx, { query: 'TARGET', limit: 10 });
    expect(len(hits[0]?.excerpt)).toBeLessThanOrEqual(120 + 'TARGET'.length);
    expect(len(hits[0]?.excerpt)).toBeGreaterThan(40);
  });

  it('folder filter excludes notes from other folders', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    // "planning" matches both "roadmap.md" body and potentially others
    const hits = await search(ctx, { query: 'planning', folder: 'projects/', limit: 10 });
    for (const hit of hits) {
      expect(hit.path).toMatch(/^projects\//);
    }
  });

  it('tag filter excludes notes without matching tag', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    // "run" matches the daily note body, but tag filter restricts to #work
    const hits = await search(ctx, { query: 'reviewed', tag: 'work', limit: 10 });
    for (const hit of hits) {
  ...
  it('limit is respected', async () => {
    ...
    const hits = await search(ctx, { query: 'note', limit: 1 });
    expect(hits.length).toBeLessThanOrEqual(1);
  });

  it('returns [] for a query with no matches', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    const hits = await search(ctx, { query: 'zzzzabsolutelyunmatchablexyz', limit: 10 });
    expect(hits).toEqual([]);
  });

  it('case-insensitive search works', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    const hitsLower = await search(ctx, { query: 'roadmap', limit: 10 });
    const hitsUpper = await search(ctx, { query: 'ROADMAP', limit: 10 });
  ...
  it('hit for a note in daily/ has the correct path', async () => {
    const ctx = buildCtx('/vault', SAMPLE_NOTES);
    const hits = await search(ctx, { query: 'morning', limit: 10 });
    expect(hits.length).toBeGreaterThan(0);
