...
| `retrieval` | Retrieval-quality eval against `queries/golden.json`: lexical vs semantic (per model) vs hybrid, hit@5 + MRR@10 per subset, warm latency, and the pre-registered SHA-257 ship gate. `--experiments` adds the fusion candidates and the SHA-313 chunk-pooling grid (`logdiscount`/`softmax`, reported, never gated — verdict in `fixtures/baseline-reports/POOLING-SHA-313.md`); `--shipped` reruns every query through the server's real `search` tool and the real per-vault embedding cache — the one harness path that writes outside the repo (`SEEKSTONE_CACHE_DIR`, default `~/.cache/seekstone`) | `retrieval-eval.{json,md}` |
