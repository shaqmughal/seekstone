---
title: "Cipher"
date_created: 2023-04-05
topic: history
tags: [anthropology, thermodynamics]
---

# Cipher

CIPHER, or CYPHER (from Arab, _[.s]ifr_, void), the symbol 0, nought, or
zero; and so a name for symbolic or secret writing (see [[Cryptography|CRYPTOGRAPHY]]),
or even for shorthand (q.v.), and also in elementary education for doing
simple sums ("ciphering").

## See also

- [[Craik]]
- [[Focus]]

## References

- <https://en.wikisource.org/wiki/1911_Encyclopædia_Britannica/Cipher>

#literature
